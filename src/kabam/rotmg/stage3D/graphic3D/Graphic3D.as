package kabam.rotmg.stage3D.graphic3D
{
   ...
   public class Graphic3D
   {
      ...
      // Per-quad batch keys for the snapshot tile sort (see sortTileRuns): the run
      // each phase-1 quad joined. Indexed by batchQuad position, written on every
      // accepted quad; only [0, tileQuads_) is ever read, at snapshot time.
      private var tileQTex_:Vector.<TextureBase> = new Vector.<TextureBase>();
      private var tileQRep_:Vector.<int> = new Vector.<int>();
      private var tileQCK_:Vector.<int> = new Vector.<int>();
      // Snapshot tile-sort scratch, sized with the batch buffers (see createBatchBuffers).
      private var tileSortOrder_:Vector.<uint>;
      private var tileSortScratch_:Vector.<Number>;
      private var tileSortBounds_:Vector.<int>;
      ...
      // Run-break diagnostics (phase-1 batching only; replayed tile-cache runs are
      // re-emitted, not re-batched). Counts which key components differed each time
      // a new run opened mid-stream: texture page/proxy, repeat program, uv offset,
      // colour transform. Reset per frame in batchBegin; read out in Renderer.
      public var runBreakTex_:int = 0;
      ...
      private function createBatchBuffers(c3d:Context3D, capacity:int) : void
      {
         ...
         this.tileSortOrder_ = new Vector.<uint>();
         this.tileSortScratch_ = new Vector.<Number>();
         this.tileSortBounds_ = new Vector.<int>();
      ...
      /**
       * True when two run-const blocks hold identical values (12 floats each).
       */
      private function tileConstsEqual(a:int, b:int) : Boolean
      {
      ...
      /**
       * Sorts the just-walked static tile prefix by batch key (texture, program,
       * constants) and rebuilds the runs, so interleaved ground variants collapse
       * into ~one run per texture instead of ~one per tile. Safe: prefix quads are
       * ground tiles with disjoint screen coverage drawn first onto clear, so their
       * relative order is invisible. Skipped when runs are already few or the order
       * is already grouped. Rewrites batchData, the run arrays and this frame's cmds
       * in place; the snapshot copy below then picks up the sorted state, and the
       * dynamic suffix keeps extending the last run as before.
       */
      private function sortTileRuns(quads:int, runs:int) : void
      {
         var q:int = 0;
         var i:int = 0;
         var f:int = 0;
         ...
         var sp:int = 0;
         var dp:int = 0;
         if(quads <= 0 || runs <= 24 || this.tileSortOrder_ == null)
         {
            return;
         }
         var ord:Dictionary = new Dictionary();
         var ordCount:int = 0;
         var order:Vector.<uint> = this.tileSortOrder_;
         order.length = quads;
         ...
         for(q = 0; q < quads; q++)
         {
            ...
            key = ((int(o) * 2 + this.tileQRep_[q]) << 20) | q;
            order[q] = key;
            if(q > 0 && key < prevKey)
            {
               sorted = false;
         ...
         order.sort(Array.NUMERIC);
         // Boundary pass (read-only over the old consts): record each new run's start.
         var bounds:Vector.<int> = this.tileSortBounds_;
         var nb:int = 0;
         bounds[nb++] = 0;
         var rc:Vector.<Number> = this.runConst;
         qa = int(order[0] & 1048575);
         ta = this.tileQTex_[qa];
         ra = this.tileQRep_[qa];
         ca = this.tileQCK_[qa];
         for(i = 1; i < quads; i++)
         {
            qb = int(order[i] & 1048575);
            if(this.tileQTex_[qb] != ta || this.tileQRep_[qb] != ra || !this.tileConstsEqual(this.tileQCK_[qb],ca))
            {
               bounds[nb++] = i;
               qa = qb;
               ta = this.tileQTex_[qb];
               ra = this.tileQRep_[qb];
               ca = this.tileQCK_[qb];
            }
         }
         bounds[nb++] = quads;
         // Permute the verts into sorted order via scratch, then copy back.
         var src:Vector.<Number> = this.batchData;
         var tmp:Vector.<Number> = this.tileSortScratch_;
         for(i = 0; i < quads; i++)
         {
            sp = int(order[i] & 1048575) * 20;
            dp = i * 20;
            for(f = 0; f < 20; f++)
            {
               tmp[dp + f] = src[sp + f];
         ...
         var total:int = quads * 20;
         for(i = 0; i < total; i++)
         {
            src[i] = tmp[i];
         }
         // Stash the old consts aside (the write pass below overwrites runConst),
         // then emit the new runs and this frame's cmds.
         var cn:int = runs * 12;
         for(i = 0; i < cn; i++)
         {
            tmp[i] = rc[i];
         }
         var newRuns:int = nb - 1;
         for(b = 0; b < newRuns; b++)
         {
            qs = int(order[bounds[b]] & 1048575);
            ck = this.tileQCK_[qs];
            this.runFirstQuad[b] = bounds[b];
            this.runQuadCount[b] = bounds[b + 1] - bounds[b];
            this.runTexture[b] = this.tileQTex_[qs];
            this.runRepeat[b] = this.tileQRep_[qs];
            dst = b * 12;
            for(c = 0; c < 12; c++)
            {
               rc[dst + c] = tmp[ck + c];
            }
            this.cmdType[b] = CMD_RUN;
            this.cmdArg[b] = b;
         }
         this.runCount = newRuns;
         this.cmdCount = newRuns;
      }
      ...
      public function batchQuad(fill:GraphicsBitmapFill) : Boolean
      {
         ...
         if(entry != null)
         {
            ...
            if(entry.tilable)
            {
               // First copy's rect with the fractional scroll offset baked into the
               // uvs (identical sampling: the shader added the same offset to the
               // same uvs, and the offset stays under one sprite so the baked quad
               // never leaves the 2x2 block). Baked offsets leave the run key, so
               // same-texture tiles with different random offsets still merge into
               // one run instead of one draw each.
               u0 = entry.u0 + fract(o0) * entry.uSpan;
               v0 = entry.v0 + fract(o1) * entry.vSpan;
               u1 = u0 + entry.uSpan;
               v1 = v0 + entry.vSpan;
               keyO0 = 0;
               keyO1 = 0;
         ...
         if(newRun && this.runOpen)
         {
            this.noteRunBreak(this.runTexture[r],this.runRepeat[r],k,texBase,repeatIdx,keyO0,keyO1,keyO2,keyO3,ct);
         }
         if(newRun)
         {
            r = this.runCount++;
         ...
         this.tileQTex_[this.batchQuads] = texBase;
         this.tileQRep_[this.batchQuads] = repeatIdx;
         this.tileQCK_[this.batchQuads] = k;
         this.batchQuads++;
         return true;
      }

      /**
       * Run-break attribution for the profiler readout (see runBreakTex_). Called
       * when a new run opens while one is already open; prevK indexes the previous
       * run's constants in runConst. One break can count several components.
       */
      private function noteRunBreak(prevTex:TextureBase, prevRep:int, prevK:int, newTex:TextureBase, newRep:int, o0:Number, o1:Number, o2:Number, o3:Number, ct:ColorTransform) : void
      {
         var rc:Vector.<Number> = this.runConst;
         if(prevTex != newTex)
         {
         ...
         if(rc[prevK] != o0 || rc[prevK + 1] != o1 || rc[prevK + 2] != o2 || rc[prevK + 3] != o3)
         {
            this.runBreakOff_++;
         }
         if(rc[prevK + 4] != ct.redMultiplier || rc[prevK + 5] != ct.greenMultiplier || rc[prevK + 6] != ct.blueMultiplier || rc[prevK + 7] != ct.alphaMultiplier
            || rc[prevK + 8] != ct.redOffset || rc[prevK + 9] != ct.greenOffset || rc[prevK + 10] != ct.blueOffset || rc[prevK + 11] != ct.alphaOffset)
         {
            this.runBreakCt_++;
         }
      }
      ...
      public function batchGeneralQuad(fill:GraphicsBitmapFill, path:GraphicsPath) : Boolean
      {
         ...
         var d:Vector.<Number> = this.batchData;
         var p:int = this.batchQuads * 20;
         // Path corners arrive in perimeter order (p0,p1,p2,p3), but the shared
         // batch index buffer triangulates slots as (0,1,2),(2,1,3), which only
         // tiles the quad when slots 1 and 2 hold opposite corners (the sprite
         // zigzag TL,TR,BL,BR). Emitting (p0,p1,p3,p2) puts the diagonal across
         // slots 1-3 so both triangles cover the whole face; in path order the
         // second triangle folds back along edge p1-p2 and drops the p3-p0
         // triangle (a quarter of every wall face).
         for(var i:int = 0; i < 4; i++)
         {
            var src:int = i == 2 ? 3 : (i == 3 ? 2 : i);
            var sx:Number = v[src * 2];
            var sy:Number = v[src * 2 + 1];
            d[p] = sx / hw + nx;
            d[p + 1] = -sy / hh + ny;
            d[p + 2] = 0;
         ...
         if(newRun && this.runOpen)
         {
            this.noteRunBreak(this.runTexture[r],this.runRepeat[r],k,texBase,repeatIdx,0,0,0,0,ct);
         }
         if(newRun)
         {
            r = this.runCount++;
